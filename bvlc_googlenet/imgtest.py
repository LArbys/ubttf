...
if __name__ == "__main__":
    ...
    image_input_node = tf.placeholder( tf.float32, name="input", shape=[batch_size,vecshape] )
    label_input_node = tf.placeholder( tf.float32,   name="label", shape=[batch_size] )

    # Load BVLC GoogLeNet Model
    caffe_weightfile = 'bvlc_googlenet.npy'
    model = BVLCGoogLeNetModel( image_input_node, label_input_node, input_dog.shape, num_classes, caffe_weightfile=caffe_weightfile )
    ...
    net_label = np.ones( label_input_node.get_shape().as_list(), dtype=np.float32 )

    # Choose image: dog or quail
    net_input[0,:] = input_dog.flatten()
    net_input[1,:] = input_quail.flatten()
